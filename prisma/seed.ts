...
async function main() {
  const adminEmail = process.env.ADMIN_SEED_EMAIL || 'admin@matchdesk.local';
  ...
  await prisma.user.upsert({
    ...
    create: { email: adminEmail, name: 'Match Desk Admin', passwordHash, role: UserRole.ADMIN },
  });
  ...
  const settings = [
    ...
    ['vip_price_usd', process.env.VIP_PRICE_USD || '9.99'],
  ];
