...
import { competitionBoard } from './config';

const demoEnabled = () => process.env.DEMO_MODE === 'true' || !process.env.DATABASE_URL;

const regionForLeague = (name: string, country = '') => {
  const n = `${name} ${country}`.toLowerCase();
  if (n.includes('uefa')) return 'UEFA';
  if (competitionBoard.topLeagues.some(x => n.includes(x.name.toLowerCase()))) return 'Europe';
  if (competitionBoard.americas.some(x => n.includes(x.name.toLowerCase()))) return 'Americas';
  if (competitionBoard.asia.some(x => n.includes(x.name.toLowerCase()))) return 'Asia';
  if (competitionBoard.africa.some(x => n.includes(x.name.toLowerCase()))) return 'Africa';
  return 'Global';
};
...
export async function homeData(){
  ...
  try {
    ...
    return {matches:fixtures.map(f=>({...f,league:f.league.name,country:f.league.country,region:regionForLeague(f.league.name,f.league.country)})),predictions:predictions.slice(0,Number(settings.find(s=>s.key==='free_prediction_count')?.value||3)).map(p=>({...p,home:p.fixture.homeTeam,away:p.fixture.away,kickoff:p.fixture.kickoff.toISOString(),league:p.fixture.league.name})),affiliate,freeCount:Number(settings.find(s=>s.key==='free_prediction_count')?.value||3),vipCount:Number(settings.find(s=>s.key==='vip_prediction_count')?.value||8),vipPrice:Number(settings.find(s=>s.key==='vip_price_usd')?.value||9.99)};
