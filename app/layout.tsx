...
export default async function RootLayout({children}:{children:React.ReactNode}){
 ...
 return <html lang="en"><body><header className="topbar"><div className="container nav"><Link className="brand" href="/"><span className="brandmark">MD</span><span>THE MATCH DESK</span></Link><nav className="navlinks"><Link href="/">Desk</Link><Link href="/matches">Matches</Link><Link href="/history">Track Record</Link><Link href="/pricing">Pro</Link></nav><div className="actions">{user?.role==='ADMIN'&&<Link className="btn" href="/admin">Admin</Link>}{user?<Link className="btn primary" href="/vip">My Pro</Link>:<Link className="btn primary" href="/login">Sign in</Link>}<MobileNav/></div></div></header>{children}<footer className="footer"><div className="container footerGrid"><div><b>THE MATCH DESK</b><br/><span>Football intelligence, probabilities and tracked predictions.</span></div><div><Link href="/about">About</Link> · <Link href="/faq">FAQ</Link> · <Link href="/privacy">Privacy</Link> · <Link href="/terms">Terms</Link></div><div>Predictions are estimates, not guarantees. 18+ · Use responsibly.</div></div></footer></body></html>;}
